...
sub new
{
	my ($class_name, $address, $port, $password, $game) = @_;
	my ($self) = {};
...
sub execute
{
	...
	if ($answer && $answer =~ /bad rcon_password/i)
	{
...
sub sendrecv
{
	...
	$splitted_answer = 0 if (!defined($splitted_answer));
	my $rs_counter = $self->{refresh_socket_counter};
	if ($rs_counter % $REFRESH_SOCKET_COUNTER_LIMIT == 0)
	{
		if ($self->{rcon_socket} && $self->{rcon_socket}->connected())
		{
		...
		if (!$self->{rcon_socket})
		{
			&::PrintEvent("TRCON", "Cannot setup TCP socket on ".$self->{address}.":".$self->{port}.": $!");
		}
		$self->{refresh_socket_counter} = 0;
		$self->{auth} = 0;
	}
	...
	if (($r_socket) && ($r_socket->connected() ))
	{
		if ($auth == 0)
		{
			...
			if ($self->__SendRcon($AUTH_PACKET_ID, $SERVERDATA_AUTH, $self->{password}, ""))
			{
				&::PrintEvent("TRCON", "Couldn't send password");
				return;
			}
			my ($id, $command, $response) = $self->__ReceiveRcon($AUTH_PACKET_ID);
		...
		if ($auth == 1)
		{
			...
			$self->__SendRcon($packet_id, $SERVERDATA_EXECCOMMAND, $msg);
			if ($splitted_answer > 0)
			{
				$self->__SendRcon($SPLIT_END_PACKET_ID, $SERVERDATA_EXECCOMMAND, "");
			}
			my ($id, $command, $response) = $self->__ReceiveRcon($packet_id, $splitted_answer);
			$self->{packet_id}++;
...
sub __SendRcon
{
	my ($self, $id, $command, $string1) = @_;
	my $data = pack("VVZ*Z*", $id, $command, $string1, "");
	my $size = length($data);
	if ($size > 4096)
	{
...
sub __ReceiveRcon
{
	# not sure wtf is going on here. passing in packet id then overwriting it
	...
	if (!$r_socket || !$r_socket->connected() || !IO::Select->new($r_socket)->can_read($TIMEOUT))
	{
		$self->{refresh_socket_counter} = 0;
		return (-1, -1, -1);
	}
...
sub GetDifficulty
{
	...
	foreach (@lines)
	{
		if ($_ =~ /^\s*"z_difficulty"\s*=\s*"([A-Za-z]+)".*$/x)
		{
